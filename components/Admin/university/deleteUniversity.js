...
import { useDeleteUniversityMutation } from "@/features/AdminApi/adminApiSlice";

const DeleteUniversity = ({ currentUniversity, onClose }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    <div className="flex justify-center gap-8 flex-col  min-h-[400px]  w-[400px] rounded">
      ...
      >
        ...
        <input
          ...
          className=" outline-none w-[100%] text-[15px] p-2 rounded border-2"
          placeholder="Enter Name of the University"
          value={`${currentUniversity.universityName}`}
          readOnly
        />
        {errors.userName && (
